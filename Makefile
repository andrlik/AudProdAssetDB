...
install:
	...
	poetry install -n
	poetry run mypy --install-types --non-interactive audprodassetdb
...
check-codestyle:
	...
	poetry run darglint --verbosity 2 audprodassetdb tests

## mypy                : Runs mypy type checking.
.PHONY: mypy
mypy:
	poetry run mypy --config-file pyproject.toml audprodassetdb
...
check-safety:
	...
	poetry run bandit -ll --recursive audprodassetdb tests
